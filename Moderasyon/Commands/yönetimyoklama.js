...
module.exports.execute = async (client, message, args, conf) => {
  const filter = message.guild.roles.cache.filter(member => message.guild.roles.cache.get(conf.enAltYonetimRolu).position <= member.position && message.guild.roles.cache.get(conf.enUstYonetimRolu).position >= member.position)
  ...
  switch (args[0]) {
    case "sıfırla": {
      message.lineReply(`**${message.guild.members.cache.filter(x => x.roles.cache.has(conf.ykatildiRolu) || x.roles.cache.has(conf.katilmadiRolu)).size}** kişiden rol alınacak.`);
      message.guild.members.cache.filter(x => x.roles.cache.has(conf.ykatildiRolu) || x.roles.cache.has(conf.katilmadiRolu)).forEach(x => x.roles.remove([conf.ykatildiRolu, conf.katilmadiRolu]).catch(err => message.lineReply(`${x} üyesinden rol alınamadı!`)));
    ...
    case "katıldı": {
      ...
      channel.members.filter(x => x.id != message.author.id && !x.user.bot).forEach(x => x.roles.add(conf.ykatildiRolu).catch(err => message.lineReply(`${x} üyesine rol verilemedi!`)));

      let katılmayanŞerefsizler = message.guild.members.cache.filter(x => !x.user.bot && !x.voice.channelID && !x.roles.cache.has(conf.ykatildiRolu) && yetkililer.some(e => x.id == e.id));
...
module.exports.configuration = {
  name: 'yönetimtoplantı',
  aliases: ["yönetimyoklama","ytoplantı"],  
  usage: 'yönetimtoplantı',
