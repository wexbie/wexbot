...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    if (!client.kullanabilir(message.author.id) && !ayar.esorumlusu.some(rol => message.member.roles.cache.has(rol))) return message.lineReply(embed.setDescription('`Yeterli Yetkin Bulunmamakta.`')).then(x => x.delete({ timeout: 5000 }));
    ...
    collector.on('collect', async (reaction) => {
        if (reaction.emoji.name == "1️⃣") {
            ...
            let newPenalty = new Penalty({
                ...
                cezaTuru: 'ETKİNLİK-CEZALI',
                cezaSebebi: reason,
                atilmaTarihi: atilisTarihi,
                bitisTarihi: bitisTarihi,
            });
            uye.roles.add(ayar.ecezalırolu).catch(console.error);
            message.lineReply(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${moment.duration(ms("7")).format('D [gün,] H [saat,] m [dakika]')}** boyunca **${reason}** nedeniyle etkinlik cezalıya atıldı`)).catch();
            client.channels.cache.find(c => c.name === ayar.ecezalılog).send(new MessageEmbed().setColor('f39c12').setDescription(`Engelleyen : ${message.author} \`(${message.author.id})\`\nEngellenen: ${uye} \`(${uye.id})\`\nSebep: ${reason}\nSüre: ${new Date(bitisTarihi).toTurkishFormatDate()} \`(${moment.duration(ms("7d")).format('D [gün,] H [saat,] m [dakika]')})\`\nCeza-i İşlem: E Cezalı \`(${cezaNumara})\``)).catch(console.error);
            newPenalty.save();
            msj.delete()
            ...
            message.lineReply(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${moment.duration(ms("14")).format('D [gün,] H [saat,] m [dakika]')}** boyunca **${reason}** nedeniyle etkinlik cezalıya atıldı`)).catch();
        ...
        message.lineReply(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${moment.duration(ms("21")).format('D [gün,] H [saat,] m [dakika]')}** boyunca **${reason}** nedeniyle etkinlik cezalıya atıldı`)).catch();
        ...
        msj.delete()
            ...
            Penalty.find({ sunucuID: message.guild.id, uyeID: uye.id }).sort({ atilmaTarihi: -1 }).exec(async (err, data) => {
                let cezalar = data.filter(d => (d.cezaTuru === "ETKİNLİK-CEZALI") && (!d.bitisTarihi || d.bitisTarihi > Date.now()))
                ...
                await uye.roles.remove(ayar.ecezalırolu).catch();
                message.lineReply(embed.setDescription(`${uye} üyesinin ${message.guild.roles.cache.get(ayar.ecezalırolu).toString()} rolü, ${message.author} tarafından alındı!`)).catch();
                if (client.channels.cache.find(c => c.name === ayar.ecezalılog)) client.channels.cache.find(c => c.name === ayar.ecezalılog).send(new MessageEmbed().setColor("GREEN").setDescription(`${uye} üyesinin ${message.guild.roles.cache.get(ayar.ecezalırolu).toString()} rolü, ${message.author} tarafından alındı!`)).catch();
                if (cezalar[0].yetkiliID === message.author.id) return
                await yetkili.send(`${message.author} yetkilisi ${uye}'ye attığın etkinlik cezalıyı kaldırdı.`).catch(err => undefined);
            });
        }
...
module.exports.configuration = {
    ...
    aliases: ["ec"],
    usage: 'ec [üye] [sebep]',
    description: 'Belirtilen üyeyi etkinlik cezalısı olarak ayarlar.',
    permLevel: 0
};