...
module.exports.execute = async (client, message, args, ayar) => {
    ...
    if (!client.kullanabilir(message.author.id) && !ayar.banciRolleri.some(rol => message.member.roles.cache.has(rol)) && !message.member.roles.cache.has(ayar.sahipyedek)) return message.react(`${client.emojis.cache.find(x => x.name === "klowraiptal")}`);
    ...
    await uye.roles.set(uye.roles.cache.has(ayar.boosterRolu) ? [ayar.underworld, ayar.boosterRolu] : [ayar.underworld]).catch(() => {
        return undefined;
    });
    if (uye.voice.channelID) uye.voice.kick().catch();
    message.lineReply(embed.setDescription(`${uye} isimli kullanıcıya ${message.author} tarafından ${message.guild.roles.cache.get(ayar.underworld).toString()} rolü verildi! ${reason ? `Sebep: ${reason}` : ''}`)).then(x => x.delete({ timeout: 15000 })).catch();
    client.channels.cache.find(c => c.name === ayar.banLogKanali).send(new MessageEmbed().setColor('BLUE').setDescription(`Engelleyen: ${message.author} \`(${message.author.id})\`\nEngellenen: ${uye.user.tag} \`(${uye.id})\`\nSebep: ${reason}\nCeza-i İşlem: Ban \`(${cezaNumara})\``)).catch(console.error);
