...
module.exports.execute = async (client, message, args, ayar, emoji) => {
     ...
     let enAltYetkiliRolu = message.guild.roles.cache.filter(member => message.guild.roles.cache.get(ayar.enAltYonetimRolu).position <= member.position && message.guild.roles.cache.get(ayar.enUstYonetimRolu).position >= member.position);
    ...
    client.splitEmbedWithDesc(`**🟢 Çevrimdışı olan yönetim görevlileri**\n\n${offlineOlanlar.map(member => `${member}`).join(", ")}`,
    ...
    client.splitEmbedWithDesc(`**🟢 Seste olup çevrimdışı olan yönetim görevlileri**\n\n${manipuleciOclar.map(member => `${member}`).join(", ")}`,
    {name: message.guild.name, icon: message.guild.iconURL({dynamic: true, size: 2048})},
    {setColor: ["2f3136"]}).then(list => { 
    list.forEach(item => {
    message.lineReply(item).catch()
...
loading.delete()
  ...
  if(args[0] && args[0].includes('ses')) {
    ...
    message.lineReply("Seste olmayan yönetim görevlileri" , { code: "xl", split: true });
  ...
  if (args[0] && (args[0].includes('online'))) {
    ...
    return  client.splitEmbedWithDesc(`**🟢 Seste olan yönetim görevlileri**\n\n${sestekiler.map(member => `${member}`).join(", ")}`,
    {name: message.guild.name, icon: message.guild.iconURL({dynamic: true, size: 2048})},
    {setColor: ["2f3136"]}).then(list => { 
    list.forEach(item => {
    message.lineReply(item)
    });
    client.splitEmbedWithDesc(`**🟢 Aktif olup seste olmayan yönetim görevlileri**\n\n${sesteOlmayanlar.map(member => `${member}`).join(", ")}`,
    {name: message.guild.name, icon: message.guild.iconURL({dynamic: true, size: 2048})},
    {setColor: ["2f3136"]}).then(list => { 
    list.forEach(item => {
    message.lineReply(item).catch()
...
module.exports.configuration = {
  name: "yönetim",
  aliases: ["mods","bildirim"],
  usage: "yönetim offline/online/ses",
