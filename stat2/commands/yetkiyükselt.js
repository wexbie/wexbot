...
module.exports = {
    ...
    async execute({ client, message, args }) {       
        ...
        if (!member.roles.cache.some(role => roles.includes(role.id))) {
           await member.roles.add(client.tasks[0].ID);
           await StatsModel.updateOne({ id: member.id }, { $set: { points: client.tasks[0].POINT, updateRoleDate: new Date()} }, { upsert: true });
           sendMessage(message, 'Kullanıcının yetkisi bulunmadığından ilk yetki verildi');
           return;
        }

        const currentTask = client.tasks.findIndex(task => member.roles.cache.has(task.ID[0]));
        const newTask = client.tasks[currentTask+1];
        if(!newTask) return sendMessage(message, 'Yetkili görevlerini bitirmiş!');
        
        const channel = message.guild.channels.cache.find(channel => channel.name === ayar.YetkiAtlamaLog)
        if(channel) {
            await channel.send(
                `${member} yetki atladın! \`Eski Yetkin:\` ${member.roles.highest.name} \`Yeni Yetkin:\` ${(member.guild.roles.cache.get(newTask.ID[0]) || { name: '@deleted-role' }).name}`
            );
        }

        const newRoles = member._roles.filter((role) => !client.tasks[currentTask].ID.includes(role)).concat(newTask.ID);
        await member.roles.set(newRoles);
        await StatsModel.updateOne({ id: member.id }, { $set: { points: newTask.POINT, updateRoleDate: new Date()} }, { upsert: true });

        sendMessage(message, `${member} adlı yetkilinin yetkisini yükselttin.`);
    }
}