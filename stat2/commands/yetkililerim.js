...
module.exports = {
    ...
    async execute({ client, message, args }) {
        ...
        const staffs = await StaffModel.find({ admin: yetkili.id });
        if (!staffs.length) return sendMessage(message, 'Yetkili bulunamadı.');
        ...
        for (const staff of staffs) {
            ...
            const StaffRole = member ? member.roles.cache.some(role => member.roles.cache.filter(rol => ekipRol.rawPosition <= rol.rawPosition).map(rol => rol.id).includes(role.id)) : false
            const user = await client.users.fetch(staff.id);
            tableArray.push([
                `${user.tag} (${user.id})`,
                staff.cinsiyet,
                new Date(staff.date).toTurkishFormatDate(),
                memberData ? (memberData.yetkili.get("erkekTeyit") || 0) + (memberData.yetkili.get("kizTeyit") || 0) : 0,
                functions.numberToString(stats.totalVoiceStats || 0),
                stats.totalChatStats || 0,
                stats2.invites || 0,
                taggeds.length,
                staffStaffs.length,
                member ? (StaffRole ? 'Yetkili' : 'Yetkili değil!') : 'Sunucuda yok!'
            ]);
        }
