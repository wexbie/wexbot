...
module.exports = {
    ...
    async execute({ client, message, args }) {
        ...
        if (member.id === message.author.id) return sendMessage(message, 'Kendini kendi yetkilin yapamazsın!')
        ...
        if (hasStaff) return sendMessage(message, `Kullanıcı zaten yetkili! \`Yetkiye başlatan kişi:\` <@${hasStaff.admin}> (\`${hasStaff.admin}\`)`);

        const question = await member.send(`${member}, **${message.author.tag}** yetkilisi seni yetkili yapmak istiyor. Kabul ediyor musun?`).catch(err => { return message.lineReply(`${member}, **${message.author.tag}** yetkilisi seni yetkili yapmak istiyor. Kabul ediyor musun?`)});
        ...
        if (collected.size > 0) {
            ...
            if(!member.roles.cache.has(config.SYSTEM.MAN_ROL_ID) && !member.roles.cache.has(config.SYSTEM.GIRL_ROL_ID)) {
                await StatsModel.updateOne({ id: message.author.id }, { $inc: { points: config.SYSTEM.STAFF_XP.BİLİNMEYEN_XP } }, { upsert: true });
            ...
            if(member.roles.cache.has(config.SYSTEM.MAN_ROL_ID)) {
                await StatsModel.updateOne({ id: message.author.id }, { $inc: { points: config.SYSTEM.STAFF_XP.MAN_XP } }, { upsert: true });
            ...
            if(member.roles.cache.has(config.SYSTEM.GIRL_ROL_ID)) {
                await StatsModel.updateOne({ id: message.author.id }, { $inc: { points: config.SYSTEM.STAFF_XP.GIRL_XP } }, { upsert: true });
                await StaffModel.create({ id: member.id, admin: message.author.id, date: Date.now(), cinsiyet: 'Kız' });
            }
            await member.roles.add(ayar.enAltYetkiliRolu);
            sendMessage(message, 'Kullanıcı yetkili olmayı kabul etti.');

            const channel = message.guild.channels.cache.find(channel => channel.name === ayar.YetkiAtlamaLog)
            if (channel) await channel.send(`${message.author}, ${member} kişisini sunucumuzda yetkili olarak görevlendirdi!`);
        }
    }
}